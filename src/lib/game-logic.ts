...
/** FNV-1a hash — deterministische shuffle-seed / deterministic shuffle seed */
...
function buildShuffledIndices(length: number, seed: number): number[] {
...
/**
 * NL: Index in een lijst voor een dag — willekeurige volgorde per cyclus, geen herhaling
 *     tot elk item één keer aan bod is geweest (zelfde antwoord voor iedereen op die dag).
 * EN: Daily list index via shuffled deck per cycle; repeats only after full pass.
 */
export function getDailyListIndex(
  dayIndex: number,
  listLength: number,
  seedSalt = 'default',
): number {
  if (listLength <= 0) return 0;
  const positionInCycle = ((dayIndex % listLength) + listLength) % listLength;
  const cycle = (dayIndex - positionInCycle) / listLength;
  const seed = hashSeed(`${seedSalt}:${cycle}:${listLength}`);
  return buildShuffledIndices(listLength, seed)[positionInCycle];
}

/** Doelgemeente voor een datum: alleen server-side gebruiken */
export function getDailyTarget(date = new Date()): Gemeente {
  const dayIndex = getDayIndex(date);
  const index = getDailyListIndex(dayIndex, GEMEENTEN.length, 'gemeentedle');
