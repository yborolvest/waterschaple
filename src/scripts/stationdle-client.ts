import { MAX_ATTEMPTS, GLOBAL_COUNT_REFRESH_MS, USE_RANDOM_TARGET_FOR_TESTING, HINT_PROVINCE_AFTER_ATTEMPT, HINT_TRAJECT_AFTER_ATTEMPT } from '../lib/stationdle/config';
import { proximityToShareEmoji } from '../lib/game-logic';
import {
  loadStationdleStats,
  saveStationdleStats,
  serializeStationGuess,
  deserializeStationGuess,
  recordStationdleResult,
  getStationdleWinRate,
} from '../lib/stationdle/stats';
...
import type {
  ...
  StationPublic,
  StationGuess,
  StationGuessApiResponse,
  StationPlayerStats,
  StationUnlockedHints,
} from '../lib/stationdle/types';

const $ = (sel: string) => document.querySelector<HTMLElement>(sel);

const state: StationGameState = {
  guesses: [],
  attempts: 0,
  gameOver: false,
  won: false,
  ...
  targetProvince: null,
  yesterday: null,
  unlockedHints: { province: null, traject: null },
};

let globalCountInterval: ReturnType<typeof setInterval> | null = null;
let lastHistoricalWins: number | null = null;

async function fetchDailyMeta(): Promise<StationDailyMeta | null> {
  try {
    const res = await fetch('/api/stationdle/daily');
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return (await res.json()) as StationDailyMeta;
  } catch (e) {
    console.warn('[Stationdle] Daily meta ophalen mislukt:', e);
    return null;
  }
}

function renderStatsUI(stats: StationPlayerStats) {
  const winRate = getStationdleWinRate(stats);
      ...
      return `
        <div class="flex items-center gap-2 text-sm">
          ...
          <div class="flex-1 h-7 bg-gray-100 rounded-md overflow-hidden">
            <div class="h-full ${isMax ? 'bg-rail-accent' : 'bg-royal-800/70'} rounded-md flex items-center justify-end pr-2 transition-all duration-500"
...
function saveTodayGame() {
  ...
  state.stats.todayGame = {
    ...
    guesses: state.guesses.map(serializeStationGuess),
  ...
  saveStationdleStats(state.stats);
}
...
function renderAttemptDots() {
  ...
  for (let i = 0; i < MAX_ATTEMPTS; i++) {
    ...
    dot.className = `w-3 h-3 rounded-full transition-all duration-300 ${
      used ? 'bg-rail-accent' : current ? 'bg-white/40 ring-2 ring-rail-accent' : 'bg-white/15'
    }`;
...
function showAutocomplete(items: StationPublic[]) {
  ...
  items.forEach((g, i) => {
    ...
    li.className = `px-4 py-2.5 cursor-pointer text-royal-950 text-sm transition-colors ${
      i === state.highlightedIndex ? 'bg-rail-accent/20' : 'hover:bg-royal-800/10'
    }`;
...
function selectStation(g: StationPublic) {
  const guessInput = $('#guess-input') as HTMLInputElement;
...
function getSelectedStation(): StationPublic | undefined {
  ...
  if (id) {
    const byId = state.stations.find((g) => g.id === id);
    if (byId && byId.name === guessInput.value.trim()) return byId;
  }
  return state.stations.find((g) => g.name.toLowerCase() === guessInput.value.trim().toLowerCase());
}
...
function mergeHints(current: StationUnlockedHints, next: StationUnlockedHints): StationUnlockedHints {
  return {
    province: next.province ?? current.province,
    traject: next.traject ?? current.traject,
  };
}
...
function renderHints() {
  ...
  const trajectUnlocked = state.attempts >= HINT_TRAJECT_AFTER_ATTEMPT && !state.won;
  ...
  provinceCard.classList.toggle('border-rail-accent/30', provinceUnlocked);
  ...
  const trajectCard = $('#hint-traject-card')!;
  const trajectLocked = $('#hint-traject-locked')!;
  const trajectText = $('#hint-traject-text')!;

  trajectCard.classList.toggle('opacity-40', !trajectUnlocked);
  trajectCard.classList.toggle('border-rail-accent/30', trajectUnlocked);
  trajectLocked.classList.toggle('hidden', trajectUnlocked);
  trajectText.classList.toggle('hidden', !trajectUnlocked);

  if (trajectUnlocked && state.unlockedHints.traject) {
    trajectText.textContent = state.unlockedHints.traject;
  }
}

async function fetchUnlockedHints(attempts: number): Promise<StationUnlockedHints | null> {
  ...
  try {
    ...
    const res = await fetch(`/api/stationdle/hints?${params}`);
    if (!res.ok) return null;
    const data = (await res.json()) as { hints: StationUnlockedHints };
    return data.hints;
  } catch {
    return null;
  }
...
function addGuessRow(guess: StationGuess, skipAnimation = false) {
  const { station, distance, arrow, proximity, isCorrect } = guess;
  ...
  row.innerHTML = `
    <div class="flex-1 min-w-0">
      <p class="font-semibold text-sm sm:text-base truncate">${station.name} <span class="text-white/40 font-normal">(${station.province})</span></p>
...
function buildShareText(): string {
  ...
  return `Stationdle #${state.puzzleNumber} ${state.won ? state.attempts : 'X'}/${MAX_ATTEMPTS} 🚉${tiles}${padding}`;
}
...
async function endGame() {
  ...
  if (!state.resultRecorded && state.stats && state.dateKey) {
    ...
    recordStationdleResult(state.stats, {
      won: state.won,
      attempts: state.attempts,
      puzzleNumber: state.puzzleNumber,
  ...
  if (state.won && lastHistoricalWins !== null) {
    ...
    historicalEl.textContent =
      lastHistoricalWins === 1
        ? 'Dit station is wereldwijd 1× als dagpuzzel correct geraden.'
        : `Dit station is wereldwijd ${lastHistoricalWins.toLocaleString('nl-NL')}× als dagpuzzel correct geraden.`;
  } else {
  ...
  if (state.won) {
    ...
    $('#result-message')!.textContent =
      `Je hebt het juiste station gevonden in ${state.attempts} ${state.attempts === 1 ? 'poging' : 'pogingen'}!${communityNote}`;
    $('#result-answer')!.classList.add('hidden');
  } else {
    $('#result-emoji')!.textContent = '🚉';
    $('#result-title')!.textContent = 'Volgende keer beter!';
    $('#result-message')!.textContent = 'Je pogingen zijn op. Het juiste station was:';
...
async function submitGuess() {
  ...
  const selected = getSelectedStation();
  if (!selected) {
  ...
  if (state.guesses.some((g) => g.station.id === selected.id)) {
    showError('Je hebt dit station al geraden.');
    return;
  }

  const nextAttempt = state.attempts + 1;

  let data: StationGuessApiResponse;
  try {
    const res = await fetch('/api/stationdle/daily', {
      method: 'POST',
    ...
    data = (await res.json()) as StationGuessApiResponse;
  } catch {
    showError('Verbinding mislukt. Controleer je internet en probeer opnieuw.');
    return;
  }

  const guess: StationGuess = {
  ...
  if (data.isCorrect) {
    lastHistoricalWins = data.stationHistoricalWins;
  }
...
function showCompletedBanner() {
  ...
  banner.className =
    'mb-4 py-3 px-4 rounded-xl bg-rail-accent/15 border border-rail-accent/30 text-center text-sm';
  if (entry?.won) {
    banner.innerHTML = `✅ Vandaag al gewonnen in <strong>${entry.attempts}</strong> ${entry.attempts === 1 ? 'poging' : 'pogingen'}! Bekijk je <button type="button" id="banner-stats-link" class="underline text-rail-light font-semibold">statistieken</button>.`;
  } else {
...
function bindEvents() {
  ...
  guessInput.addEventListener('input', () => {
    delete guessInput.dataset.selectedId;
    hideError();
    const items = filterStations(guessInput.value);
  ...
  guessInput.addEventListener('focus', () => {
    if (guessInput.value.trim()) {
      showAutocomplete(filterStations(guessInput.value));
    }
  });
  ...
  guessInput.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (state.highlightedIndex >= 0 && items[state.highlightedIndex]) {
        selectStation(items[state.highlightedIndex]);
      }
      void submitGuess();
...
export async function initStationdle() {
  renderGlobalSolveCount(null, 'loading');
  ...
  state.globalSolveCount = meta.globalSolveCount;
  state.stats = loadStationdleStats();
  ...
  if (canRestore && state.stats.todayGame) {
    ...
    state.guesses = tg.guesses.map(deserializeStationGuess).filter((g): g is StationGuess => g !== null);
    ...
    state.unlockedHints = tg.unlockedHints ?? { province: null, traject: null };
  } else {
    state.guesses = [];
    state.attempts = 0;
    ...
    state.unlockedHints = { province: null, traject: null };
  }

  if (
    state.attempts >= HINT_PROVINCE_AFTER_ATTEMPT &&
    !state.won &&
    (!state.unlockedHints.province ||
      (state.attempts >= HINT_TRAJECT_AFTER_ATTEMPT && !state.unlockedHints.traject))
  ) {
  ...
  if (meta.yesterday) {
    ...
    $('#yesterday-station-name')!.textContent = meta.yesterday.name;
    $('#yesterday-station-province')!.textContent = ` (${meta.yesterday.province})`;
  }
  ...
  if (state.guesses.length === 0 && !state.gameOver) {
    ...
    empty.textContent = 'Nog geen pogingen. Kies een station en druk op Raad.';
