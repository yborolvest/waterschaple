...
import { SNELWEGEN, sortSnelwegen, type Snelweg } from '../data/snelwegen';
...
import {
  ...
  serializeSnelwegGuess,

  deserializeSnelwegGuess,

  recordSnelwegdleResult,

  getSnelwegdleWinRate,

} from '../lib/snelwegdle/stats';

import { formatGlobalSolveText } from '../lib/global-counter';

import { openModal, closeModal as closeA11yModal } from '../lib/modal-a11y';

import type { SnelwegdleGameState, SnelwegGuess, SnelwegdlePlayerStats } from '../lib/snelwegdle/types';
...
const state: SnelwegdleGameState = {
...
function renderStatsUI(stats: SnelwegdlePlayerStats) {

  const winRate = getSnelwegdleWinRate(stats);
      ...
      return `
        ...
        <div class="flex items-center gap-2 text-sm">
          ...
          <div class="flex-1 h-7 bg-gray-100 rounded-md overflow-hidden">
            ...
            <div class="h-full ${isMax ? 'bg-highway-accent' : 'bg-royal-800/70'} rounded-md flex items-center justify-end pr-2 transition-all duration-500"
...
function saveTodayGame() {
  ...
  state.stats.todayGame = {
    ...
    guesses: state.guesses.map(serializeSnelwegGuess),
...
function renderAttemptDots() {
  ...
  for (let i = 0; i < MAX_ATTEMPTS; i++) {
    ...
    dot.className = `w-3 h-3 rounded-full transition-all duration-300 ${
      ...
      used ? 'bg-highway-accent' : current ? 'bg-white/40 ring-2 ring-highway-accent' : 'bg-white/15'
...
function filterSnelwegen(query: string): Snelweg[] {

  const q = query.trim().toLowerCase();

  const sorted = sortSnelwegen(SNELWEGEN);
...
function showAutocomplete(items: Snelweg[]) {
  ...
  items.forEach((s, i) => {
    ...
    li.className = `px-4 py-2.5 cursor-pointer text-royal-950 text-sm transition-colors ${
      ...
      i === state.highlightedIndex ? 'bg-highway-accent/20' : 'hover:bg-royal-800/10'

    }`;

    li.innerHTML = `<span class="font-bold">${s.name}</span> <span class="text-gray-500">${s.route}</span>`;
...
function selectSnelweg(s: Snelweg) {

  const guessInput = $('#guess-input') as HTMLInputElement;

  guessInput.value = formatSnelwegLabel(s);

  guessInput.dataset.selectedId = s.id;
...
function getSelectedSnelweg(): Snelweg | undefined {
  ...
  if (id) {
    ...
    const byId = SNELWEGEN.find((s) => s.id === id);

    if (byId && guessInput.value.trim() === formatSnelwegLabel(byId)) return byId;

  }

  const value = guessInput.value.trim();

  const byName = SNELWEGEN.find((s) => s.name.toLowerCase() === value.toLowerCase());

  if (byName) return byName;

  const byLabel = SNELWEGEN.find((s) => formatSnelwegLabel(s).toLowerCase() === value.toLowerCase());
...
function renderHints() {
  ...
  const missing = $('#hint-route-missing')!;
...
function addGuessRow(guess: SnelwegGuess, skipAnimation = false) {

  const { snelweg, distance, arrow, proximity, isCorrect } = guess;
  ...
  row.innerHTML = `
    ...
    <div class="flex-1 min-w-0">
      ...
      <p class="font-semibold text-sm sm:text-base truncate">${snelweg.name}</p>

      <p class="text-white/50 text-xs mt-0.5 truncate">${snelweg.route} · ${Math.round(distance)} km</p>
...
function buildShareText(): string {
  ...
  return `Snelwegdle #${state.puzzleNumber} ${state.won ? state.attempts : 'X'}/${MAX_ATTEMPTS} 🛣️${tiles}${padding}`;
...
async function endGame() {
  ...
  if (!state.resultRecorded && state.stats && state.dateKey) {
    ...
    recordSnelwegdleResult(state.stats, {
  ...
  if (state.won) {
    ...
    $('#result-message')!.textContent =
      ...
      `Je hebt de juiste snelweg gevonden in ${state.attempts} ${state.attempts === 1 ? 'poging' : 'pogingen'}!${communityNote}`;
    ...
    $('#result-message')!.textContent = 'Je pogingen zijn op. De juiste snelweg was:';
...
function submitGuess() {
  ...
  const selected = getSelectedSnelweg();

  if (!selected) {

    showError('Kies een geldige snelweg uit de lijst.');
  ...
  if (state.guesses.some((g) => g.snelweg.id === selected.id)) {

    showError('Je hebt deze snelweg al geraden.');
  ...
  const guess: SnelwegGuess = { snelweg: selected, distance, bearing, arrow, proximity, isCorrect };
...
function showCompletedBanner() {
  ...
  banner.className =
    ...
    'mb-4 py-3 px-4 rounded-xl bg-highway-accent/15 border border-highway-accent/30 text-center text-sm';

  if (entry?.won) {

    banner.innerHTML = `✅ Vandaag al gewonnen in <strong>${entry.attempts}</strong> ${entry.attempts === 1 ? 'poging' : 'pogingen'}! Bekijk je <button type="button" id="banner-stats-link" class="underline text-highway-light font-semibold">statistieken</button>.`;

  } else {

    banner.innerHTML = `❌ Vandaag al gespeeld. Het antwoord was <strong>${state.target ? formatSnelwegLabel(state.target) : 'onbekend'}</strong>. Morgen een nieuwe puzzel!`;
...
function bindEvents() {
  ...
  guessInput.addEventListener('input', () => {
    ...
    const items = filterSnelwegen(guessInput.value);
  ...
  guessInput.addEventListener('focus', () => {
    ...
    if (guessInput.value.trim()) {
      ...
      showAutocomplete(filterSnelwegen(guessInput.value));
  ...
  guessInput.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (state.highlightedIndex >= 0 && items[state.highlightedIndex]) {
        ...
        selectSnelweg(items[state.highlightedIndex]);
...
export function initSnelwegdle() {
  ...
  state.stats = loadSnelwegdleStats();
  ...
  if (import.meta.env.DEV && state.target) {
    ...
    console.log('[Snelwegdle debug] Doel:', formatSnelwegLabel(state.target));
  ...
  const yesterday = getYesterdaySnelwegTarget(today);
  ...
  $('#yesterday-snelweg-name')!.textContent = formatSnelwegLabel(yesterday);
  ...
  if (state.guesses.length === 0 && !state.gameOver) {
    ...
    empty.textContent = 'Nog geen pogingen. Kies een snelweg en druk op Raad.';
